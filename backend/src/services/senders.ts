  ...
  for (let i = 0; i < wanted; i++) {
    try {
      const creds = env.ethereal.autoProvision
        ...
        : { smtp_host: env.ethereal.host, smtp_port: env.ethereal.port, smtp_user: env.ethereal.user, smtp_pass: env.ethereal.pass, from_email: env.ethereal.user };
      if (!creds.smtp_user || !creds.smtp_pass) { logger.warn('senders', 'no SMTP creds'); break; }
      const row = await queryOne<SenderRow>(
        `INSERT INTO senders (user_id, label, from_email, smtp_host, smtp_port, smtp_user, smtp_pass, hourly_limit) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    ...
    } catch (err) { logger.error('senders', 'failed to provision', err); break; }
  }
  return created;
}
